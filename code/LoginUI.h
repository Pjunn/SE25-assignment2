#pragma once

#include <string>
#include <fstream>

using namespace std;

class Login; // Forward declaration of Login

/*
    클래스 이름 : LoginUI
    기능       : 회원 로그인 UI를 담당하는 클래스
    멤버 변수  : Login* control      - Login 컨트롤 객체 포인터
                ifstream& in_fp     - 입력 파일 스트림 참조
                ofstream& out_fp    - 출력 파일 스트림 참조
                string id           - 입력받은 회원 ID
                string password     - 입력받은 회원 비밀번호
*/
class LoginUI {
private:
    Login* control;      // Login 컨트롤 객체 포인터
    ifstream& in_fp;     // 입력 파일 스트림
    ofstream& out_fp;    // 출력 파일 스트림
    string id;           // 입력받은 회원 ID
    string password;     // 입력받은 회원 비밀번호
public:
    /*
        함수 이름 : LoginUI::LoginUI
        기능     : LoginUI 객체를 생성하고, 멤버 변수들을 초기화함
        전달 인자 : ifstream& in_fp - 입력 파일 스트림 참조
                   ofstream& out_fp - 출력 파일 스트림 참조
        반환값   : 없음 (생성자)
    */
    LoginUI(ifstream& in_fp, ofstream& out_fp);

    /*
        함수 이름 : LoginUI::startInterface
        기능     : 컨트롤 객체를 연결하고 입력 스트림에서 로그인 정보를 읽어옴
        전달 인자 : Login* control - 컨트롤 객체 포인터
        반환값   : 없음
    */
    void startInterface(Login* control);

    /*
        함수 이름 : LoginUI::login
        기능     : 로그인 기능을 수행하고 결과를 출력 파일에 기록함
        전달 인자 : 없음
        반환값   : 없음
    */
    void login();
};
